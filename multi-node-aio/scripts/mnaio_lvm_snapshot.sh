...
set -xe
export DYN_INV_FILE=`cat /tmp/dynamic_inventory.txt`
ansible -i ${DYN_INV_FILE} 'galera_all:!galera_all[0]' -m service -a 'name=mysql state=stopped'
ansible -i ${DYN_INV_FILE} 'rabbitmq_all:!rabbitmq_all[0]' -m service -a 'name=rabbitmq-server state=stopped'
EOF
...
# Snapshot all nodes
######################
echo "Starting vm snapshots"
for CURNODE in $(virsh list --all | awk '/shut off/{print $2}') ; do

  if [ ! -e "/dev/vg01/${CURNODE}_snap" ] ; then
    echo "lvcreate -c 512k -s -n ${CURNODE}_snap -L 20G /dev/vg01/${CURNODE} > /dev/null 2>&1"
    lvcreate -c 512k -s -n ${CURNODE}_snap -L 20G /dev/vg01/${CURNODE} > /dev/null 2>&1
    if [ $? != 0 ] ; then
        echo "lvcreate failed.  Please investigate"
        exit 255
    fi
    sleep 5
  fi
...
ansible -i ${DYN_INV_FILE} 'galera_all[0]' -m shell -a 'ps -p $(cat /var/lib/mysql/${GALERA_CONTAINER}.pid) > /dev/null 2>&1; if [ $? != 0 ] ; then /etc/init.d/mysql start --wsrep-new-cluster; fi' > /dev/null 2>&1
EOF
...
export DYN_INV_FILE=`cat /tmp/dynamic_inventory.txt`
export GALERA_CONTAINER=`cat /tmp/galera_container.txt`
ansible -i ${DYN_INV_FILE} 'galera_all[0]' -m shell -a 'ps -p $(cat /var/lib/mysql/${GALERA_CONTAINER}.pid) > /dev/null 2>&1; if [ $? == 0 ] ; then kill -SIGQUIT $(cat /var/lib/mysql/${GALERA_CONTAINER}.pid); fi' > /dev/null 2>&1
ansible -i ${DYN_INV_FILE} 'galera_all[0]' -m service -a 'name=mysql state=started'
EOF
